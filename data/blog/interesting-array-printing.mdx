...
- 创建一个空的二维数组 res=[[ ],[ ]...]
- 先利用 for 循环创建正序数组放入刚刚创建好的二维数组中
- 即[ [1,2,3,4,5]....[31,32,32,34,35] ]
- 创建过程中寻找到 res 中的下标为奇数的项数进行反转，此时 res=[[1,2,3,4,5],[10,9,8,7,6]...]
- 此时创建新的二维数组 newres=[[ ],[ ]...] ，再对原二维数组 res 进行 90 度旋转并将元素放入 newres 中去
- 此时 newres=[ [1,10,11,20,21,26,31]...],此时进行解构打印即可
...
原因为 fill()的参数不是基本类型时，比如数组、对象，并不是将它的值填充到数组，而是将它的地址填充到数组，所以填充后数组的每一项都指向的是同一个对象(类似于浅拷贝)，会出现"牵一发而动全身"的现象
